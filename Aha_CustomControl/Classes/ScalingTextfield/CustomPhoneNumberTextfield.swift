...
//  AhaFood
//
//  Created by DuongNgoAnh on 11/6/17.
...
open class CustomPhoneNumberTextfield: ScalingMFTextField, UITextFieldDelegate {
    ...
    open func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        // For specific case, user click on keyboard predictive to input their device phone number.
        // In this case, this method sho string == " " and range is zero range.
        ...
        let filteredCharacters = modifiedTextField.filter {
            return  String($0).rangeOfCharacter(from: (textField as! CustomPhoneNumberTextfield).nonNumericSet as CharacterSet) == nil
        }
    ...
    // For the case phone number enter from iOS keyboard predictive.
    // After entering phone number, the text must be re-formatted.
    @objc open func textFieldDidChange(_ textField: UITextField) {
        guard let wrappedText = textField.text else {
            return
        }
        textField.text = partialFormatter.formatPartial(wrappedText)
    }
